...
# Colorscheme
declare -r COLOR_DEFAULT='#f8f8f8'
...
_draw_menu() {
  ...
  local labels=("$@")
  btui::color "$text_color"
  for ((i = 0; i < ${#labels[@]}; i++)); do
    if [[ $curr == "$i" ]]; then
      y=$((y+i)) addstr "$(btui::color "$selected_color")$indicator ${labels[i]}$(btui::color "$text_color")"
    else
...
_imenu() {
  ...
  while true; do
    ...
    y=$((y+curr)) addstr "$(btui::color $COLOR_SELECTED)>"
    y=$((y+prev)) _addstr_rgbtrans "  ${labels[prev]}" _RGBTRANS_MENU_DESELECT 0.2 &
    y=$((y+curr)) x=$((x+2)) _addstr_rgbtrans "${labels[curr]}" _RGBTRANS_MENU_SELECT
  done
  indicator=' ' text_color=\#969696 _draw_menu "${labels[@]}"
  # shellcheck disable=SC2034
  outvar="${labels[curr]}"
}
